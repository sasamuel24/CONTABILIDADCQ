...
type EstadoUI = 'Borrador' | 'En validacion' | 'En revision' | 'Devuelto' | 'Aprobado' | 'En tesoreria' | 'Pagado';
...
const estadoConfig: Record<EstadoUI, { label: string; bg: string; text: string; border: string; icon: React.FC<{ className?: string }> }> = {
  ...
  'En validacion': { label: 'Esperando validador', bg: '#f5f3ff', text: '#7c3aed', border: '#ddd6fe', icon: ShieldCheck },
...
// ============================================================
// Pipeline visual — Tarjeta Comercial (con paso de validador)
// ============================================================

function PipelineComercial({ estado }: { estado: EstadoUI }) {
  const pasos = [
    { key: 'Borrador',      label: 'Borrador' },
    { key: 'En validacion', label: 'Validador' },
  ...
  const indexActual =
    estado === 'Borrador'      ? 0 :
    estado === 'En validacion' ? 1 :
    estado === 'Devuelto'      ? 1 :
    estado === 'En revision'   ? 2 :
    estado === 'Aprobado'      ? 3 :
    estado === 'En tesoreria'  ? 5 : 6;
        ...
        return (
          <div key={paso.key} className="flex items-center flex-1">
            <div className="flex flex-col items-center" style={{ minWidth: 52 }}>
              ...
              >
                {devuelto && paso.key === 'En validacion' ? 'Devuelto' : paso.label}
              </span>
            </div>
            {i < pasos.length - 1 && (
              <div
...
// ============================================================
// DetallePaquete Comercial
// ============================================================

function DetallePaqueteComercial({
  paqueteId,
  onCerrar,
}: {
  paqueteId: string;
  ...
  const handleEnviar = async () => {
    ...
    try {
      ...
      toast.success('Paquete enviado a validación del responsable.');
      onCerrar();
    } catch (err: any) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-5">
      <div className="rounded-2xl p-5 border border-gray-100"
            ...
            {(esBorrador || esDevuelto) ? (
              <>
                ...
                <button onClick={handleEnviar} disabled={saving}
                  ...
                  {esDevuelto ? 'Reenviar corregido' : 'Enviar a validación'}
                </button>
              </>
            ) : (
            ...
            <div className="flex items-center gap-4 mt-2 flex-wrap">
              ...
              {paquete.aprobador && (
                <span className="flex items-center gap-1.5 text-xs text-gray-400">
                  ...
                  Gerente: <strong className="text-gray-600">{paquete.aprobador.nombre}</strong>
                </span>
              )}
            </div>
          </div>
        ...
        <div className="mt-5 pt-4 border-t border-gray-100">
          <p className="text-xs text-gray-400 uppercase tracking-wider mb-3"
            style={{ fontFamily: 'Neutra Text Demi, Montserrat, sans-serif' }}>
            Flujo Tarjeta Comercial
          </p>
          <PipelineComercial estado={estadoUI} />
        </div>
      </div>
      ...
      {['En validacion', 'En revision', 'Aprobado', 'Pagado'].includes(estadoUI) && (
        <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4">
          <Info className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
          <p className="text-sm text-blue-600" style={{ fontFamily: 'Neutra Text Book, Montserrat, sans-serif' }}>
            {estadoUI === 'En validacion' && 'Esperando la validación del responsable. Tras validar, se enviará el correo de aprobación al gerente comercial.'}
            {estadoUI === 'En revision' && `Validado. Esperando aprobación del gerente${paquete.aprobador ? ` (${paquete.aprobador.nombre})` : ''}. Se envió un correo con el enlace de aprobación (válido 72 horas).`}
      ...
      {(esBorrador || esDevuelto) && (
        <div className="pb-4">
          <button onClick={handleEnviar} disabled={saving}
            ...
            {esDevuelto ? 'Reenviar paquete corregido' : 'Enviar a validación del responsable'}
          </button>
        </div>
      )}
...
// ============================================================
// NuevoPaqueteComercialForm
// ============================================================

function NuevoPaqueteComercialForm({
  onCerrar,
  onCreado,
}: {
  onCerrar: () => void;
  ...
  const handleGuardar = async () => {
    ...
    try {
      paquete = await createPaqueteGasto(semana, hijoId || undefined);
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-gray-900" style={{ fontFamily: 'Neutra Text Bold, Montserrat, sans-serif' }}>
            Nuevo paquete de gastos comercial
          </h3>
          <p className="text-sm text-gray-400 mt-0.5" style={{ fontFamily: 'Neutra Text Book, Montserrat, sans-serif' }}>
            Registra los gastos realizados con la tarjeta comercial y adjunta los soportes
          </p>
        </div>
        ...
        <div className="flex items-start gap-4 flex-wrap">
          ...
          <div className="flex items-start gap-2 bg-amber-50 border border-amber-100 rounded-xl p-3 flex-1 min-w-[200px]">
            ...
            <p className="text-xs text-amber-700">
              Al enviar, el paquete pasa primero por el <strong>validador (responsable)</strong> y luego al gerente comercial para aprobación.
            </p>
          </div>
        </div>
...
// ============================================================
// PaqueteCard Comercial
// ============================================================

function PaqueteCardComercial({ p, onClick }: { p: PaqueteListItem; onClick: () => void }) {
...
// ============================================================
// Página principal ComercialPage
// ============================================================
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/70 text-xs" style={{ fontFamily: 'Neutra Text Book, Montserrat, sans-serif' }}>
                Legalización de gastos con tarjeta comercial
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-gray-900" style={{ fontFamily: 'Neutra Text Bold, Montserrat, sans-serif' }}>
                {vista === 'historial' ? 'Historial de envios' : 'Mis Paquetes Tarjeta Comercial'}
              </h2>
              <p className="text-xs text-gray-400 mt-0.5" style={{ fontFamily: 'Neutra Text Book, Montserrat, sans-serif' }}>
                {vista === 'historial'
                  ? 'Paquetes enviados para validación, aprobación o pagados'
                  : 'Registra y legaliza los gastos realizados con tu tarjeta comercial'}
              </p>
            </div>
            {vista === 'lista' && (
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
          ...
          {vista === 'nuevo' && (
            <NuevoPaqueteComercialForm onCerrar={irALista}
              onCreado={(id) => { setPaqueteActivo(id); setVista('detalle'); }} />
          )}

          {vista === 'detalle' && paqueteActivo && (
            <DetallePaqueteComercial paqueteId={paqueteActivo} onCerrar={irALista} />
          )}
          ...
          {vista === 'historial' && (
            <div className="max-w-2xl mx-auto">
              ...
              {!loading && historial.length > 0 && (
                <div className="space-y-3">
                  {historial.map(p => <PaqueteCardComercial key={p.id} p={p} onClick={() => { setPaqueteActivo(p.id); setVista('detalle'); }} />)}
                </div>
              )}
            </div>
          )}
                ...
                <>
                  {conGastosDevueltos.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-3">
                        {conGastosDevueltos.map(p => <PaqueteCardComercial key={p.id} p={p} onClick={() => { setPaqueteActivo(p.id); setVista('detalle'); }} />)}
                      </div>
                    </div>
                  )}
                  {activos.length > 0 && (
                    <div>
                      ...
                      <div className="space-y-3">
                        {activos.map(p => <PaqueteCardComercial key={p.id} p={p} onClick={() => { setPaqueteActivo(p.id); setVista('detalle'); }} />)}
                      </div>
                    </div>
                  )}
                  {historial.length > 0 && (
                  ...
                  {activos.length === 0 && historial.length === 0 && conGastosDevueltos.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                      ...
                      <p className="text-sm text-gray-400 mb-6 max-w-xs" style={{ fontFamily: 'Neutra Text Book, Montserrat, sans-serif' }}>
                        Crea tu primer paquete de gastos con tarjeta comercial para comenzar la legalización.
                      </p>
                      <button onClick={() => { setVista('nuevo'); setPaqueteActivo(null); }}
                        ...
                        <Plus className="w-4 h-4" /> Crear mi primer paquete comercial
                      </button>
                    </div>
                  )}
                </>
