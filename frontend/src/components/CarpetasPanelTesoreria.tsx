...
interface CarpetasPanelTesoreriaProps {
  onSelectCarpeta: (carpeta: CarpetaTesoreria | null) => void;
  selectedCarpeta: CarpetaTesoreria | null;
}

export function CarpetasPanelTesoreria({ onSelectCarpeta, selectedCarpeta }: CarpetasPanelTesoreriaProps) {
  ...
  const handleUpdateFolder = async (carpetaId: string, newName: string) => {
    ...
    try {
      await updateCarpetaTesoreria(carpetaId, { nombre: newName.trim() });
      setEditingId(null);
      await loadCarpetas();
    } catch (err) {
  ...
  const handleDeleteFolder = async (carpetaId: string, carpetaNombre: string) => {
    ...
    try {
      await deleteCarpetaTesoreria(carpetaId);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      >
        ...
        <span style={{fontFamily: 'Neutra Text Demi, Montserrat, sans-serif'}} className="text-sm font-medium text-gray-900">Facturas Sin Archivar</span>
      </div>
