          ...
          <ul className="p-3 space-y-2">
            {missing.map(m => (
              <li key={m} className="flex items-start gap-2">
                <span className="mt-0.5 w-4 h-4 rounded-full bg-amber-100 border border-amber-300 flex items-center justify-center shrink-0">
...
// Dos motivos distintos comparten esta insignia: la devolución de Contabilidad
// (naranja) y el rechazo del aprobador desde el correo (rojo). Misma mecánica,
// distinto color y textos.
const VARIANTES_MOTIVO = {
  devuelta: {
    etiqueta: 'Devuelta',
    titulo: 'Factura devuelta',
    autorLabel: 'Devuelta por:',
    fondo: '#fff7ed',
    borde: '#fed7aa',
    texto: '#c2410c',
    claseBorde: 'border-orange-200',
    claseBordeSuave: 'border-orange-100',
    claseTitulo: 'text-orange-800',
    claseIcono: 'text-orange-600',
    claseCerrar: 'text-orange-300 hover:text-orange-600',
    claseAutorLabel: 'text-orange-700',
    claseAutor: 'text-orange-900',
  },
  rechazada: {
    etiqueta: 'Rechazada',
    titulo: 'Rechazada en la aprobación',
    autorLabel: 'Rechazada por:',
    fondo: '#fef2f2',
    borde: '#fecaca',
    texto: '#b91c1c',
    claseBorde: 'border-red-200',
    claseBordeSuave: 'border-red-100',
    claseTitulo: 'text-red-800',
    claseIcono: 'text-red-600',
    claseCerrar: 'text-red-300 hover:text-red-600',
    claseAutorLabel: 'text-red-700',
    claseAutor: 'text-red-900',
  },
} as const;
...
function MotivoBadge({
  ...
  motivo: string;
  autor?: string | null;
  variante?: keyof typeof VARIANTES_MOTIVO;
}) {
  const v = VARIANTES_MOTIVO[variante];
  const [open, setOpen] = useState(false);
  ...
  return (
    <>
      ...
      {open && createPortal(
        <div
          ref={popRef}
          className={`fixed bg-white border ${v.claseBorde} rounded-xl shadow-2xl overflow-hidden`}
...
// ── Filtro tipo Excel para la columna "Area Receptora" (filtrar por tienda) ──
function AreaFilterPopover({
  areas,
  selected,
  onChange,
}: {
  ...
  return (
    <>
      <button
        ...
        style={{ color: active ? '#00829a' : '#9ca3af' }}
      >
        <Filter className="w-3.5 h-3.5" fill={active ? '#00829a' : 'none'} />
      </button>

      {open && createPortal(
        <div
          ref={popRef}
          className="fixed bg-white border border-gray-200 rounded-xl shadow-2xl overflow-hidden flex flex-col"
          style={{ top: pos.top, left: pos.left, zIndex: 9999, width: 290, maxHeight: 380, fontFamily: FONT }}
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-100 bg-gray-50">
            <Filter className="w-3.5 h-3.5 text-[#00829a]" />
            <span className="text-xs font-semibold text-gray-700">Filtrar por tienda</span>
            <button onClick={() => setOpen(false)} className="ml-auto text-gray-300 hover:text-gray-600">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between px-3 py-1.5 text-xs border-b border-gray-100">
            <button className="text-[#00829a] hover:underline font-medium" onClick={() => onChange(new Set(areas))}>
              Seleccionar todo
            </button>
            <button className="text-gray-500 hover:underline font-medium" onClick={() => onChange(new Set())}>
              Limpiar
            </button>
          </div>
          <div className="overflow-y-auto py-1">
            {filtered.length === 0 ? (
              ...
              <label key={a} className="flex items-center gap-2 px-3 py-1.5 hover:bg-gray-50 cursor-pointer">
                <input
                  type="checkbox"
                  checked={selected.has(a)}
                  onChange={() => toggle(a)}
                  className="rounded border-gray-300 text-[#00829a] focus:ring-[#00829a]"
                />
                <span className="text-sm text-gray-700 truncate">{a}</span>
              </label>
            ))}
          </div>
        </div>,
...
export function InboxView() {
  ...
  // Filtro por tienda (columna "Area Receptora"). Vacío = todas.
...
const itemsPerPage = 20;
  ...
  const filteredFacturas = facturas.filter(f => {
    ...
    const matchesArea = areaFilter.size === 0 || areaFilter.has(f.area);
    return matchesSearch && matchesStatus && matchesTab && matchesArea;
  });

  // Tiendas presentes en la bandeja, para el filtro tipo Excel de "Area Receptora".
  const areasDisponibles = Array.from(new Set(facturas.map(f => f.area).filter(Boolean)))
    .sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));

  const handleAreaFilterChange = (next: Set<string>) => { setAreaFilter(next); setCurrentPage(1); };
  ...
  return (
    <div className="h-full">
                  ...
                  <tr
                    ...
                    className={`transition-colors cursor-pointer ${
                      factura.motivo_rechazo_email
                        ? 'border-l-4 border-red-400 bg-red-50/40 hover:bg-red-50/70'
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        {factura.motivo_rechazo_email && (
                          <MotivoBadge
                            variante="rechazada"
                            motivo={factura.motivo_rechazo_email}
                            autor={factura.rechazado_por_nombre}
                          />
                        )}
                        {factura.motivo_devolucion && (
                          <MotivoBadge
                            motivo={factura.motivo_devolucion}
                            autor={factura.devuelta_por_nombre}
                          />
                        )}
                      </div>
                    </td>
      ...
      {selectedFactura && (
        <>
          {/* Responsable (y Responsable de Tiendas) usan el componente de Responsable */}
          {user?.role === 'responsable' || user?.role === 'responsable_tiendas' ? (
