...
from modules.anticipos.schemas import AnticipioCreate, AnticipioOut, AnticipioListResponse
...
async def listar_anticipos(
    ...
    return await svc.listar(skip, limit, estado)
...
@router.post(
    ...
    summary="Crear anticipo y paquete de gastos asociado",
)
async def crear_anticipo(
    ...
    role = user.role.code.lower() if user.role else ""
    if role not in ROLES_TESORERIA:
        raise HTTPException(status_code=403, detail="Solo Tesorería puede crear anticipos.")
...
@router.get(
    ...
    response_model=AnticipioListResponse,
    summary="Anticipos asignados al usuario actual",
)
async def mis_anticipos(
    ...
    """Retorna los anticipos donde el usuario autenticado es el beneficiario asignado."""
...
async def get_anticipo(
    ...
    role = user.role.code.lower() if user.role else ""
    if role not in ROLES_TESORERIA:
        raise HTTPException(status_code=403, detail="Solo Tesorería puede ver anticipos.")
    return await svc.get_by_id(anticipo_id)


@router.patch(
    "/anticipos/{anticipo_id}/cerrar",
    response_model=AnticipioOut,
    summary="Cerrar un anticipo (Tesorería)",
)
