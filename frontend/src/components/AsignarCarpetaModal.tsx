...
import { X, Folder, Check, ChevronRight, ChevronDown, FolderOpen, Plus, Edit2, Trash2, FolderPlus, Search } from 'lucide-react';
...
export function AsignarCarpetaModal({ isOpen, onClose, factura, onSuccess }: AsignarCarpetaModalProps) {
  ...
  // Filtra el árbol por coincidencia de nombre (mantiene padres con hijos coincidentes)
  const carpetaMatchesBusqueda = (c: Carpeta, q: string): boolean => {
    if (c.nombre.toLowerCase().includes(q)) return true;
    return c.children?.some(ch => carpetaMatchesBusqueda(ch, q)) ?? false;
  };

  const filterTree = (list: Carpeta[], q: string): Carpeta[] => {
    if (!q) return list;
    return list
  ...
  const carpetasFiltradas = useMemo(
    () => filterTree(carpetas, busqueda.toLowerCase().trim()),
    [carpetas, busqueda]
  );
  ...
  const renderCarpeta = (carpeta: Carpeta, level: number = 0): React.ReactElement => {
    const isExpanded = expandedFolders.has(carpeta.id) || (!!busqueda.trim() && carpeta.children && carpeta.children.length > 0);
      ...
      <div key={carpeta.id}>
        <div
          ...
          style={{
            ...
            borderLeft: isSelected ? '3px solid #00829a' : '3px solid transparent',
          }}
        >
          {/* Zona EXPANDIR: chevron + ícono de carpeta — área amplia y clara */}
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); if (hasChildren) toggleFolder(carpeta.id); }}
            className="flex items-center gap-1.5 pl-3 pr-2 py-2.5 shrink-0 focus:outline-none"
            style={{ cursor: hasChildren ? 'pointer' : 'default', minWidth: 52 }}
            tabIndex={hasChildren ? 0 : -1}
            title={hasChildren ? (isExpanded ? 'Contraer' : 'Expandir') : undefined}
          >
            {hasChildren ? (
              isExpanded
            ...
            {isExpanded
              ? <FolderOpen className="w-4 h-4 shrink-0" style={{color: '#00829a'}} />
              : <Folder className="w-4 h-4 shrink-0" style={{color: '#00829a'}} />}
          </button>

          {/* Zona SELECCIONAR: solo el nombre */}
          <button
            type="button"
            className="flex-1 text-left py-2.5 pr-2 text-sm font-medium text-gray-900 hover:text-[#00829a] transition-colors truncate focus:outline-none"
            style={{ fontFamily: 'Neutra Text Book, Montserrat, sans-serif', background: 'none', border: 'none' }}
            onClick={(e) => { e.stopPropagation(); handleSelectCarpeta(carpeta); }}
                ...
                {!isLoading && carpetas.length > 0 && (
                  <div className="relative mb-2">
                    <input
                      ...
                      onFocus={(e) => { e.target.style.borderColor = '#00829a'; e.target.style.boxShadow = '0 0 0 2px rgba(0,130,154,0.15)'; }}
                  ...
                  <div className="border border-gray-300 rounded-lg max-h-80 overflow-y-auto bg-white">
                    {carpetas.length === 0 ? (
                      ...
                      <div className="p-6 text-center">
                        ...
                        <p style={{fontFamily: 'Neutra Text Book, Montserrat, sans-serif'}} className="text-sm text-gray-500">
                          Sin resultados para <span className="font-semibold">"{busqueda}"</span>
                        </p>
                      </div>
                    ) : (
                      <div className="p-1">
                        {carpetasFiltradas.map((carpeta) => renderCarpeta(carpeta))}
                      </div>
                    )}
                  </div>
                )}
